...
function searchDB(db, term) {
  return unique(searchItems(db, term).concat(searchFileNames(db, term)))
}

function searchFileNames(db, term) {
  // Only return files, directories should already be expanded by searching files
  return db.files.filter(f => f.includes(term) && f[f.length - 1] != '/')
}

function searchItems(db, term) {
  let results = db.items.filter(item => {
...
function unique(as) {
  return as.filter((x, index, arr) => {
    // dedupe by taking only where elements's index matches first index
